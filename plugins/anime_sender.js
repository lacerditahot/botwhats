...
Asena.addCommand({pattern: 'animepic ?(.*)', fromMe: wk, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { caption: '*camilo 👨‍💻*' })
    ...
    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { caption: '*camilo 👨‍💻*' })
  } else {
...
Asena.addCommand({pattern: 'animegif ?(.*)', fromMe: wk, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(gif_buffer.data), MessageType.video, { caption: '*camilo 👨‍💻*', mimetype: Mimetype.gif })
  } else if (match[1] == 'nsfw') {
    ...
    await message.sendMessage(Buffer.from(gif_buffer_nsfw.data), MessageType.video, { caption: '*camilo 👨‍💻*', mimetype: Mimetype.gif })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));
